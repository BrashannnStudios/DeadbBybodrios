...
def is_admin(member: discord.Member, bot) -> bool:
    config = bot.bot_configs.get(member.guild.id, {})
    admin_roles = set(config.get("admin_roles", []))
    member_roles = {r.id for r in member.roles}
    return bool(member_roles & admin_roles) or member.guild_permissions.administrator
...
class Moderation(commands.Cog):
    ...
    async def lock(self, ctx: commands.Context, channel: Optional[discord.TextChannel] = None):
        if not is_staff_or_admin(ctx.author, self.bot):
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} No tienes permisos para usar este comando.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Canal {channel.mention} bloqueado.",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)

        log_embed = discord.Embed(
            title=f"{AVISO} Canal bloqueado",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
    ...
    async def unlock(self, ctx: commands.Context, channel: Optional[discord.TextChannel] = None):
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Canal {channel.mention} desbloqueado.",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)

        log_embed = discord.Embed(
            title=f"{AVISO} Canal desbloqueado",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
        log_embed.add_field(name="Canal", value=channel.mention, inline=True)
    ...
    async def ban(self, ctx: commands.Context, member: discord.Member, *, reason: str = "No especificada"):
        ...
        if member.top_role >= ctx.author.top_role and not ctx.author.guild_permissions.administrator:
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} No puedes banear a alguien con rol igual o superior.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} {member.mention} ha sido baneado.\n**Razón:** {reason}",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)

        log_embed = discord.Embed(
            title=f"{AVISO} Usuario baneado",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
    ...
    async def tempban(self, ctx: commands.Context, member: discord.Member, time: str, *, reason: str = "No especificada"):
        ...
        if not duration:
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} Formato de tiempo inválido. Usa: `30s`, `5m`, `2h`, `1d`, `1w`",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} {member.mention} ha sido baneado temporalmente por **{time}**.\n**Razón:** {reason}",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)

        log_embed = discord.Embed(
            title=f"{AVISO} Usuario baneado temporalmente",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
        log_embed.add_field(name="Usuario", value=f"{member} (`{member.id}`)", inline=True)
    ...
    async def unban(self, ctx: commands.Context, user: Union[discord.User, int], *, reason: str = "No especificada"):
        ...
        try:
            ...
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} Ese usuario no está baneado.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Usuario `{user_id}` desbaneado.\n**Razón:** {reason}",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)

        log_embed = discord.Embed(
            title=f"{AVISO} Usuario desbaneado",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
    ...
    async def kick(self, ctx: commands.Context, member: discord.Member, *, reason: str = "No especificada"):
        ...
        if member.top_role >= ctx.author.top_role and not ctx.author.guild_permissions.administrator:
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} No puedes expulsar a alguien con rol igual o superior.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} {member.mention} ha sido expulsado.\n**Razón:** {reason}",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)

        log_embed = discord.Embed(
            title=f"{AVISO} Usuario expulsado",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
        log_embed.add_field(name="Usuario", value=f"{member} (`{member.id}`)", inline=True)
    ...
    async def mute(self, ctx: commands.Context, member: discord.Member, time: str, *, reason: str = "No especificada"):
        ...
        if duration.total_seconds() > 28 * 86400:
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} El timeout máximo de Discord es 28 días.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} {member.mention} ha sido silenciado por **{time}**.\n**Razón:** {reason}",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)

        log_embed = discord.Embed(
            title=f"{AVISO} Usuario silenciado (Timeout)",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
        log_embed.add_field(name="Usuario", value=f"{member} (`{member.id}`)", inline=True)
    ...
    async def unmute(self, ctx: commands.Context, member: discord.Member, *, reason: str = "No especificada"):
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Se ha removido el timeout de {member.mention}.\n**Razón:** {reason}",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)

        log_embed = discord.Embed(
            title=f"{AVISO} Timeout removido",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
        log_embed.add_field(name="Usuario", value=f"{member} (`{member.id}`)", inline=True)
    ...
    async def warn(self, ctx: commands.Context, member: discord.Member, *, reason: str = "No especificada"):
        ...
        log_embed = discord.Embed(
            title=f"{AVISO} Advertencia aplicada",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
        log_embed.add_field(name="Usuario", value=f"{member} (`{member.id}`)", inline=True)
    ...
    async def warnings(self, ctx: commands.Context, member: Optional[discord.Member] = None):
        ...
        if member != ctx.author and not is_staff_or_admin(ctx.author, self.bot):
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} No tienes permisos para ver las advertencias de otros.",
                color=SYSTEM_COLOR
            ))
        ...
        if not warns:
            return await ctx.send(embed=discord.Embed(
                description=f"{LUPA} {member.mention} no tiene advertencias.",
                color=SYSTEM_COLOR
            ))

        embed = discord.Embed(
            title=f"{LUPA} Advertencias de {member}",
            color=SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
    ...
    async def delwarn(self, ctx: commands.Context, member: discord.Member, index: int):
        ...
        if not doc or not doc.get("warns"):
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} Ese usuario no tiene advertencias.",
                color=SYSTEM_COLOR
            ))
        ...
        if index < 1 or index > len(warns):
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} Índice inválido. Usa un número entre 1 y {len(warns)}.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Advertencia #{index} eliminada de {member.mention}.\n**Razón eliminada:** {removed['reason']}",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)
    ...
    async def editreason(self, ctx: commands.Context, member: discord.Member, index: int, *, new_reason: str):
        ...
        if index < 1 or index > len(warns):
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} Índice inválido.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Razón de la advertencia #{index} actualizada.\n**Antes:** {old}\n**Ahora:** {new_reason}",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)
    ...
    async def note(self, ctx: commands.Context, member: discord.Member, *, content: str):
        ...
        await self.bot.db.notes.update_one(
            {"guild_id": ctx.guild.id, "user_id": member.id},
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Nota añadida a {member.mention}.",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)
    ...
    async def viewnotes(self, ctx: commands.Context, member: discord.Member):
        ...
        if not notes:
            return await ctx.send(embed=discord.Embed(
                description=f"{LUPA} {member.mention} no tiene notas.",
                color=SYSTEM_COLOR
            ))

        embed = discord.Embed(
            title=f"{PLUMA} Notas de {member}",
            color=SYSTEM_COLOR
        )
    ...
    async def delnote(self, ctx: commands.Context, member: discord.Member, index: int):
        ...
        if not doc or not doc.get("notes"):
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} Ese usuario no tiene notas.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Nota #{index} eliminada de {member.mention}.",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)
    ...
    async def slowmode(self, ctx: commands.Context, seconds: int, channel: Optional[discord.TextChannel] = None):
        ...
        if seconds < 0 or seconds > 21600:
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} El slowmode debe estar entre 0 y 21600 segundos.",
                color=SYSTEM_COLOR
            ))
        ...
        embed = discord.Embed(
            description=f"{ACEPTAR} Slowmode de {channel.mention} establecido en **{seconds}s**.",
            color=SYSTEM_COLOR
        )
        await ctx.send(embed=embed)
    ...
    async def clear(self, ctx: commands.Context, amount: int):
        ...
        if amount < 1 or amount > 100:
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} Debes especificar un número entre 1 y 100.",
                color=SYSTEM_COLOR
            ))

        deleted = await ctx.channel.purge(limit=amount + 1)
        embed = discord.Embed(
            description=f"{ACEPTAR} Se eliminaron **{len(deleted)-1}** mensajes.",
            color=SYSTEM_COLOR
        )
...
class Utility(commands.Cog):
    ...
    async def dm(self, ctx: commands.Context, member: discord.Member, *, message: str):
        ...
        try:
            ...
            await ctx.send(embed=discord.Embed(
                description=f"{ACEPTAR} Mensaje enviado a {member.mention}.",
                color=SYSTEM_COLOR
            ))
        except discord.Forbidden:
            await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} No pude enviar el DM (el usuario tiene los DMs cerrados).",
                color=SYSTEM_COLOR
            ))
    ...
    async def addrole(self, ctx: commands.Context, member: discord.Member, role: discord.Role):
        ...
        if role >= ctx.author.top_role and not ctx.author.guild_permissions.administrator:
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} No puedes asignar un rol igual o superior al tuyo.",
                color=SYSTEM_COLOR
            ))

        await member.add_roles(role, reason=f"Por {ctx.author}")
        await ctx.send(embed=discord.Embed(
            description=f"{ACEPTAR} Rol {role.mention} añadido a {member.mention}.",
            color=SYSTEM_COLOR
        ))
    ...
    async def removerole(self, ctx: commands.Context, member: discord.Member, role: discord.Role):
        ...
        await ctx.send(embed=discord.Embed(
            description=f"{ACEPTAR} Rol {role.mention} removido de {member.mention}.",
            color=SYSTEM_COLOR
        ))
    ...
    async def nick(self, ctx: commands.Context, member: discord.Member, *, nickname: str = None):
        ...
        if nickname:
            await ctx.send(embed=discord.Embed(
                description=f"{ACEPTAR} Nickname de {member.mention} cambiado a **{nickname}**.",
                color=SYSTEM_COLOR
            ))
        else:
            await ctx.send(embed=discord.Embed(
                description=f"{ACEPTAR} Nickname de {member.mention} reseteado.",
                color=SYSTEM_COLOR
            ))
    ...
    async def userinfo(self, ctx: commands.Context, member: Optional[discord.Member] = None):
        ...
        embed = discord.Embed(
            title=f"{LUPA} Información de {member}",
            color=member.color if member.color.value != 0 else SYSTEM_COLOR,
            timestamp=datetime.now(timezone.utc)
        )
...
class Tickets(commands.Cog):
    ...
    async def adduser(self, ctx: commands.Context, member: discord.Member):
        ...
        if not ticket:
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} Este canal no es un ticket.",
                color=SYSTEM_COLOR
            ))

        await ctx.channel.set_permissions(member, view_channel=True, send_messages=True, read_message_history=True)
        await ctx.send(embed=discord.Embed(
            description=f"{ACEPTAR} {member.mention} ha sido añadido al ticket.",
            color=SYSTEM_COLOR
        ))
    ...
    async def removeuser(self, ctx: commands.Context, member: discord.Member):
        ...
        await ctx.send(embed=discord.Embed(
            description=f"{ACEPTAR} {member.mention} ha sido removido del ticket.",
            color=SYSTEM_COLOR
        ))
    ...
    async def close(self, ctx: commands.Context):
        ...
        if not is_staff_or_admin(ctx.author, self.bot) and ctx.author.id != ticket.get("user_id"):
            return await ctx.send(embed=discord.Embed(
                description=f"{DENEGADO} No tienes permisos para cerrar este ticket.",
                color=SYSTEM_COLOR
            ))
        ...
        await self.bot.db.tickets.update_one(
            ...
            {"$set": {"closed": True, "closed_by": ctx.author.id, "closed_at": datetime.now(timezone.utc)}}
        )
    ...
    async def delete_ticket(self, ctx: commands.Context):
        ...
        await ctx.send(embed=discord.Embed(
            description=f"{RELOJ_ARENA} Eliminando ticket en 5 segundos...",
            color=SYSTEM_COLOR
        ))
    ...
    async def rename(self, ctx: commands.Context, *, new_name: str):
        ...
        await ctx.send(embed=discord.Embed(
            description=f"{ACEPTAR} Ticket renombrado a **{new_name}**.",
            color=SYSTEM_COLOR
        ))
